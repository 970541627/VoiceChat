...
public class PointerFree {
    ...
    public static void freeServerPointer() {
        Iterator<Pointer> iterator = pointerServerIntegerMap.keySet().iterator();
        while (iterator.hasNext()) {
            Memory first = (Memory) iterator.next();
            Integer pd = pointerServerIntegerMap.get(first);
            if (pd == null) {
                continue;
            }
            if (pd > SERVER_MAX_COUNT) {
                iterator.remove();
